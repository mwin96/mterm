...
case $OSTYPE in
  darwin*)
    zipdir=MTerm-macos-$TAG_NAME
    if [[ "$BUILD_REASON" == "Schedule" ]] ; then
      zipname=MTerm-macos-nightly.zip
    else
      zipname=$zipdir.zip
    fi
    rm -rf $zipdir $zipname
    mkdir $zipdir
    cp -r assets/macos/MTerm.app $zipdir/
    # Omit MetalANGLE for now; it's a bit laggy compared to CGL,
    # and on M1/Big Sur, CGL is implemented in terms of Metal anyway
    rm $zipdir/MTerm.app/*.dylib
    mkdir -p $zipdir/MTerm.app/Contents/MacOS
    mkdir -p $zipdir/MTerm.app/Contents/Resources
    cp -r assets/shell-integration/* $zipdir/MTerm.app/Contents/Resources
    cp -r assets/shell-completion $zipdir/MTerm.app/Contents/Resources
    tic -xe wezterm -o $zipdir/MTerm.app/Contents/Resources/terminfo termwiz/data/wezterm.terminfo
    ...
    for bin in wezterm wezterm-mux-server wezterm-gui strip-ansi-escapes ; do
      ...
      if [[ -f $TARGET_DIR/release/$bin ]] ; then
        cp $TARGET_DIR/release/$bin $zipdir/MTerm.app/Contents/MacOS/$bin
      else
        ...
        lipo $TARGET_DIR/*/release/$bin -output $zipdir/MTerm.app/Contents/MacOS/$bin -create
      fi
    done
    ...
    if [ -n "$MACOS_TEAM_ID" ] ; then
      ...
      /usr/bin/codesign --keychain build.keychain --force --options runtime \
        --entitlements ci/macos-entitlement.plist --deep --sign "$MACOS_TEAM_ID" $zipdir/MTerm.app/

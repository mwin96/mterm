...
hook_mode() {
  local input tool_args command description command_b64 description_b64 modified_args runner
  ...
  command=$(jq -r '.command // empty' <<<"$tool_args" 2>/dev/null) || return 0
  [[ -n "$command" && "$command" != *"wezterm-shell-pane.sh --run"* ]] || return 0

  description=$(jq -r '.description // empty' <<<"$tool_args" 2>/dev/null)
  ...
  runner="$HOME/.copilot/hooks/wezterm-shell-pane.sh"

  modified_args=$(
    jq -c \
      --arg command "$runner --run '$command_b64' '$description_b64'" \
      '.command = $command' <<<"$tool_args" 2>/dev/null
  ) || return 0

  jq -cn --argjson modifiedArgs "$modified_args" '{modifiedArgs: $modifiedArgs}'
}
...
run_mode() {
  ...
  output_file=$(mktemp "${TMPDIR:-/tmp}/copilot-wezterm-shell.XXXXXX") || {
    /bin/bash -c "$command"
    return $?
  }
  ...
  focus_lock="${TMPDIR:-/tmp}/copilot-wezterm-focus-${UID}.lock"
